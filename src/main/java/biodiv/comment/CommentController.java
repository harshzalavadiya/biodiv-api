...
public class CommentController {
	...
	public String addComment(@QueryParam("commentId") Long commentId,@QueryParam("commentBody") String commentBody,@QueryParam("tagUserId") String tagUserId,
			...
			@QueryParam("rootHolderId") Long rootHolderId,@QueryParam("rootHolderType") String rootHolderType,
			@QueryParam("mainParentId") Long mainParentId,@QueryParam("parentId") Long parentId,@QueryParam("subject") String subject,
		...
		if(commentBody !=null && commentBody.trim().length()>0){
			msg = commentService.addComment(commentId,commentBody,tagUserId,commentHolderId,commentHolderType,rootHolderId,rootHolderType,
					mainParentId,parentId,subject,commentType,newerTimeRef,commentPostUrl,userLang,Long.parseLong(profile.getId()));
