...
public class ObservationService extends AbstractService<Observation> {
	...
	public Map<String,Object> getRecommendationVotes(String obvs) {
		...
		try{
			long[] obvIds = Arrays.asList(obvs.split(",")).stream().map(String::trim).mapToLong(Long::parseLong).toArray();
			...
			if(noOfObvs == 1){
				...
				query = "select * from reco_vote_details rv where rv.observation_id in ("+obvs+")  order by rv.observation_id";
			}
			...
			for(Map<String,Object> recoVote : recoVotes){
				...
				Map<String,Object> obvRecoVotesResult = (Map<String, Object>) obvListRecoVotesResult.get(((Long)recoVote.get("observationId")).toString());
				if(obvRecoVotesResult == null){
					obvRecoVotesResult = new HashMap<String,Object>();
					obvRecoVotesResult.put("recoVotes", new ArrayList<Map<String,Object>>()); //checkin g%%%%%%%%%%%%%%%%%%%%%%%%%
				...
				if(recoVote.get("commonNameRecoId") != null){
					...
					Recommendation cnReco =  recommendationService.findById(((java.math.BigInteger) recoVote.get("commonNameRecoId")).longValue());
					Long cnLangId = (cnReco.getLanguageId() != null)?(cnReco.getLanguageId()):(Long)205L; ///  what is englishId hack
					if(!((Map<String,Object>)map.get("commonNames")).containsKey(cnLangId.toString())){
						((Map<String, Object>) map.get("commonNames")).put(cnLangId.toString(),new HashSet<Recommendation>());
					}
					Set<Recommendation> abc = (Set<Recommendation>) ((Map<String, Object>) map.get("commonNames")).get(cnLangId.toString());
					abc.add(cnReco);
				...
				Map<String,Object> author = userService.findAuthorSignature(userService.findById((Long) recoVote.get("authorId")));
				//User originalAuthor = userService.findById((Long) recoVote.get("originalAuthor"));
				...
				map.put("hasObvLockPerm", hasObvLockPerm((Long)recoVote.get("observationId"),(Long)recoVote.get("recoId")));
				Long totalCommentCount = commentService.getTotalRecoCommentCount((Long)map.get("recoId"),(Long)recoVote.get("observationId"));
			...
			for(Map.Entry<String,Object> entry : obvListRecoVotesResult.entrySet()){
				...
				for(Map<String,Object> entry2 : a){
					...
					if(map.get("commonNames") != null ){
						...
						// to be written
						String cNames = Observation.getFormattedCommonNames((Map<String, Object>) map.get("commonNames"),true);
...
//							if(((User)authors_1.get(0)).getId() == currentUser.id){
...
//						}
				...
				obvRecoVotesResult.put("recoVotes", finalRecos); // have to sort later
			}
	...
	private Boolean hasObvLockPerm(Long obvId, Long recoId) {
		Taxon taxon = (recommendationService.findById(recoId) != null)?recommendationService.findById(recoId).getTaxonConcept():null;
